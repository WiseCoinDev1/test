...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: osiris.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: Osirisd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizeosirisamount=<n> " + _("Keep N Osiris anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Osiris is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Osiris is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Osiris version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "Osiris server starting\n");
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Osiris") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Osiris to complete") << "\n";
    ...
    nAnonymizeOsirisAmount = GetArg("-anonymizeosirisamount", 0);
    if(nAnonymizeOsirisAmount > 999999) nAnonymizeOsirisAmount = 999999;
    if(nAnonymizeOsirisAmount < 2) nAnonymizeOsirisAmount = 2;
    ...
    LogPrintf("Anonymize Osiris Amount %d\n", nAnonymizeOsirisAmount);

    /* Denominations
       A note about convertability. Within Darksend pools, each denomination
       is convertable to another.
       For example:
       1OSI+1000 == (.1OSI+100)*10
       10OSI+10000 == (1OSI+1000)*10
    */
