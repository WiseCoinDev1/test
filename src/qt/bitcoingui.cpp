...
{
    resize(900, 520);
    setWindowTitle(tr("Osiris") + " - " + tr("Wallet"));
#ifndef Q_OS_MAC
...
#endif
    setObjectName("osiris");
    setStyleSheet("#osiris { background-color: qradialgradient(cx: -0.8, cy: 0, fx: -0.8, fy: 0, radius: 1.4, stop: 0 #dedede, stop: 1 #efefef);  }");
    // Accept D&D of URIs
    setAcceptDrops(true);
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setToolTip(tr("Send coins to a Osiris address"));
    ...
    aboutAction = new QAction(tr("&About Osiris"), this);
    aboutAction->setToolTip(tr("Show information about Osiris"));
    aboutAction->setMenuRole(QAction::AboutRole);
    aboutQtAction = new QAction(tr("About &Qt"), this);
    aboutQtAction->setToolTip(tr("Show information about Qt"));
    aboutQtAction->setMenuRole(QAction::AboutQtRole);
    optionsAction = new QAction(tr("&Options..."), this);
    optionsAction->setToolTip(tr("Modify configuration options for Osiris"));
...
void BitcoinGUI::createToolBars()
{
    ...
    toolbar->setStyleSheet("QToolButton { color: #000000; font-size: 14px; font-family: Georgia,Times,Times New Roman,serif; font-weight: 400; font-variant: small-caps; padding: 3px; border: none;}"
                           "QToolButton:hover { background-color: #C0EBED; border: none; padding-top: 3px; padding-bottom: 3px; }"
                           "QToolButton:checked { background-color: #ADD8E6; border: none; padding-top: 3px; padding-bottom: 3px; }"
                           "QToolButton:pressed { background-color: #C0EBED; border: none; padding-top: 3px; padding-bottom: 3px; }"
    ...
    header->setMargin(10);
    //header->setStyleSheet("QLabel {padding: 0px -5px 10px -15px;}");
    header->setSizePolicy(QSizePolicy::Fixed, QSizePolicy::Fixed);
    header->setPixmap(QPixmap(":/images/header"));
    header->setMaximumSize(142,162);
...
void BitcoinGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if(clientModel)
    {
        ...
        if(clientModel->isTestNet())
        {
            ...
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("Osiris client") + QString(" ") + tr("[testnet]"));
...
void BitcoinGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("Osiris client"));
...
void BitcoinGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to Osiris network", "", count));
}
...
void BitcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid Osiris address or malformed URI parameters."));
    }
