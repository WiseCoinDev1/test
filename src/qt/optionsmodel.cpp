...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeOsirisAmount"))
        settings.setValue("nAnonymizeOsirisAmount", 1000);
    nAnonymizeOsirisAmount = settings.value("nAnonymizeOsirisAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeOsirisAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeOsirisAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeOsirisAmount:
            return QVariant(nAnonymizeOsirisAmount);
        default:
            return QVariant();
        }
    }
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeOsirisAmount:
            nAnonymizeOsirisAmount = value.toInt();
            settings.setValue("nAnonymizeOsirisAmount", nAnonymizeOsirisAmount);
            emit AnonymizeOsirisAmountChanged(nAnonymizeOsirisAmount);
            break;
        default:
            break;
        }
