...
//
// Bootup the masternode, look for a 7500 OSI input and register on the network
//
void CActiveMasternode::ManageStatus()
{
    ...
    if(status == MASTERNODE_NOT_PROCESSED) {
        ...
        if(GetMasterNodeVin(vin, pubKeyCollateralAddress, keyCollateralAddress)) {
            ...
            /* rewards are not supported in osiris.conf */
...
{
    ZOsiriscoinAddress address(collateralAddress);
